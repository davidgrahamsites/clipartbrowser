...
### 2026-06-23T01:00:00Z · win-zh · claude
- Changed: Merged `main` into zh-CN and translated the two 2026-06-23 Windows-EN
  UI features. (1) Browser bar split into tab row + tool row — label "搜索引擎：".
  (2) Inspect-image mode — toggle "🔍 查看" (title "添加前先全尺寸查看图片（检查水印等）"),
  modal title "查看图片", buttons "使用此图片" / "跳过", plus translated status lines.
  Logic (handlePick/commitPick split, showInspect/closeInspect/useInspect, state)
  merged byte-for-byte from EN. Bumped → 0.2.3.
- Where: `windows/src/index.html`, `windows/src/renderer.js`, `windows/package.json`.
